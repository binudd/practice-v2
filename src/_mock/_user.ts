...
// Cycle through the 5 roles so the seeded list exercises every code path.
const pickRole = (index: number): UserRole => ALL_ROLES[index % ALL_ROLES.length];
...
/** A canonical mock user per role - handy for dev sign-in and testing. */
export const _roleUsers: Record<UserRole, (typeof _userList)[number]> = {
  superadmin: { ..._userList[0], role: 'superadmin' },
  admin: { ..._userList[1], role: 'admin' },
  manager: { ..._userList[2], role: 'manager' },
  member: { ..._userList[3], role: 'member' },
  client: { ..._userList[4], role: 'client' },
};
